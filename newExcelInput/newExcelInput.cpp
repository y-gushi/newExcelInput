...
int main(char* fname[], int i) {
    ...
    /*-----------------------
     シェアー文字列読み込み
     -----------------------*/
    std::ifstream fr(infile2, std::ios::in | std::ios::binary);

    char shares[] = "sharedStrings.xml";//ファイル位置　ファイル名部分一致検索　sharedStrings.xml
    bool fla = false;
    DeflateDecode* decShare = new DeflateDecode(&fr);//sharestring ファイルの保存用
    ...
    DeflateDecode* decsheet = new DeflateDecode(&fr);
    ...
    shareシート読み込み
    -----------------------*/
    std::ifstream frt(infile, std::ios::in | std::ios::binary);
    ...
    decShare = new DeflateDecode(&frt);
    ...
    while (hr2->filenum < hr2->ER->centralsum) {
        ...
        if (cddata) {
            ...
            Hdeco = new DeflateDecode(&frt);//解凍
